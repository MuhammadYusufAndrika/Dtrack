...
import { LayoutDashboard, Route, Bell, Menu, ChevronLeft, LogOut, Truck } from 'lucide-react';
            ...
            <aside className={`fixed top-0 left-0 z-50 h-screen bg-dark-900/95 backdrop-blur-xl border-r border-dark-700/50 transition-all duration-300 flex flex-col ${collapsed ? 'w-[72px]' : 'w-[260px]'} ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="flex items-center justify-between p-4 border-b border-dark-700/50">
                    <Link href="/driver" className="flex items-center gap-3 min-w-0">
                        ...
                        {!collapsed && <div className="min-w-0"><h1 className="text-sm font-bold text-dark-50 truncate">FleetVision</h1><p className="text-[10px] text-primary-400 font-medium">Driver</p></div>}
                    </Link>
                ...
                <div className="p-4 border-t border-dark-700/50">
                    <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-dark-400 hover:text-dark-200 hover:bg-dark-800/50 transition-all w-full">
                        ...
                        {!collapsed && <div className="min-w-0 text-left"><p className="text-sm font-medium text-dark-200 truncate">Driver</p><p className="text-xs text-dark-400 truncate">Sign Out</p></div>}
                    </button>
                </div>
            </aside>
                ...
                <header className="sticky top-0 z-30 bg-dark-900/80 backdrop-blur-xl border-b border-dark-700/50">
                    <div className="flex items-center justify-between px-4 lg:px-6 py-3">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-xs text-dark-400">Driver</span>
                        </div>
                    </div>
                </header>
